...
const SamJRLatest = ({ selectedAgent }) => {
...
*Structure all responses with clarity, breaking down complex concepts, and ensuring each step includes necessary compliance checks, regulatory references, or further inquiries where relevant.* Now respond to the user's query based on this framework.
    ...
    try {
          ...
          messages:
            type === "withoutTrainingData"
              ? [
                  ...
                  {
                    role: "user",
                    content: `Now i am going to provide some data of uncategorized transactions for this business.Provide a complete list of all categorized transactions directly in the response without any summarization or previews, regardless of the size. Include all original columns along with the new columns: Chart of Transaction Particulars, Account, Account Category, Account Type, Statement, and Transaction Partner. Present the response in a JSON array format containing each transaction as an object, and ensure no row is omitted.`,
                  },
                  message,
                  // {
                  //   role: "assistant",
      ...
      if (type === "Generate For Me" || type === "uploadpayee") {
        ...
        console.log(botMessage?.content, "response from luca" + type);
                    ...
                    >
                      ...
                      <input
                        ...
                        onChange={async (event) => {
                          let csvData = await convertCSVToJson(event);
                    ...
                    extractAccountNames(sampleFileJson)?.length > 0 && (
                      <div className={classNames.contentInput}>
                        ...
                        <div className={classNames.allAccounts}>
                          ...
                          <AddNewItem
                            stateValue={sampleFileJson}
                            setState={setSampleFileJson}
                          />
                        </div>
                      </div>
                    )}
...
const TransactionsTable = ({ data, searchTerm, setData }) => {
  ...
  const handleInputChange = (rowIndex, key, value) => {
    ...
    // Find the original key in data that corresponds to the header
    const normalizedKey = key.toLowerCase().replace(" ", "_");

    // Look up if there’s an alternative key format in the original data
    const originalKey = Object.keys(data[rowIndex]).find(
      (k) => k.toLowerCase().replace(" ", "_") === normalizedKey
    );

    // Update the value in the original data structure
    if (originalKey) {
...
const AddNewItem = ({ stateValue, setState }) => {
        ...
        onClick={() => {
          if (allVisible) {
            if (stateValue?.length > 0) {
              const keyWithName = Object.keys(stateValue[0]).find((key) =>
                key.toLowerCase().includes("name")
              );
              setState((prev) => {
                return [...prev, { [keyWithName]: addItem }];
              });
            }
            setAddVisible(false);
            setAddItem("");
